...
def main(argv):
  ...
  for _ in range(M):
    ...
    S = vr.ReadIntList(C, 1, N)
    assert len(S) == len(set(S)), 'Found duplicates in S'
  assert sum_C <= 200000, 'Found sum(C) > 200000'
  ...
  for _ in range(Q):
    ...
    if type == 1:
      i = vr.ReadInt(1, N)
      assert vr.ReadChar() == ' ', 'Input is not space separated'
      X = vr.ReadInt(1, 10000)
    else:
      j = vr.ReadInt(1, M)
      ask_exists = True
    vr.ReadEndl()

  assert ask_exists, 'Did not find an operation of the second type'
